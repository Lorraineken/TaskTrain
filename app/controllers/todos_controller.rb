   ...
   def create 
    ...
    def update
       ...
       if todo 
        ...
        app_response(message:'failed', data: {info: 'something went wrong.could not update'}, status: :unprocessable_entity)
       end
    end

    def destroy 
        user.todos.find(params[:id]).destroy 
        app_response(message:'success', data: {info : 'deletes todo successfully'},status: 204)
    end
